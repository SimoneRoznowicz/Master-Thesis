    ...
    V: Serialize + Clone,
{
    ...
    pub fn from_proof_to_bytes(proof: Proof) -> &[u8]{  //[tag, direction, 32_hash_bytes..., direction, 32_hash_bytes,...]
    ...
    pub fn from_bytes_to_proof(vec: Vec<u8>) -> Proof {  //[direction, 32_hash_bytes..., direction, 32_hash_bytes,...]
        let siblings: Vec<Sibling> = Vec::new();
        let len_hash: u8 = 32;
        let mut i = 0;
        while(i<vec.len()){
            let direction = vec[i];
            let hash = from_bytes(vec[i+1..i+1+len_hash]);
